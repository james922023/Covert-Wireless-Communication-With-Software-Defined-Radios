...
# Define the spreading sequence
spreading_sequence = np.array([1, -1, 1, -1])

# Spread the symbols
spread_symbols = np.array([symbol * spreading_sequence for symbol in x_symbols]).flatten()




samples = np.concatenate((start_sequence, spread_symbols)) # FOR THE GRAPH 0 is positive, 1 is negative
...
if samples_after_barker < (num_symbols * 2 * len(spreading_sequence))-1:
...
#print("extracted",extracted_samples)



# Despread the received samples
despread_samples = np.array([extracted_samples[i:i + len(spreading_sequence)] * spreading_sequence for i in range(0, len(extracted_samples), len(spreading_sequence))]).flatten()



# Convert the complex array based on the real part
if peak_value>0:
    converted_array = np.where(despread_samples.real > 0, 0, 1)
    #print(converted_array)
else:
    converted_array = np.where(despread_samples.real > 0, 1, 0)
    #print(converted_array)
    
# Remove redundancy (take every second element)
reduced_array = converted_array[::8]
